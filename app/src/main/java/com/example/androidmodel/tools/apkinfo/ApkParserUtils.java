...
public class ApkParserUtils {
    ...
    private final JSONObject shellJson;
    ...
    public ApkParserUtils(Context context, String apkPath) {
        ...
        shellJson = getShellJsonFromAsset(context,"features_shell.json");
    }
    ...
    /**
     * @return 1.获取到 apk 压缩文件中 AM 文件上次修改的时间, timestamp ms;
...
//        sdf.setTimeZone(TimeZone.getTimeZone("UTC"));
    ...
    private Set<String> getShellInfo_() {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                if(shellJson.has(name)){
                    stubInfo.add(shellJson.getString(name));
                }
            }
    ...
    private JSONObject getShellJsonFromAsset(Context context, String fileName) {
        JSONObject shellJson = new JSONObject();
        ...
        try {
            ...
            reader.close();
            shellJson = new JSONObject(jsonString.toString());
